<template>
  <div class="home">
    <header>
      <h1>2048</h1>
      <button class="init-button" @click="init">新游戏</button>
      <p>
        总分: <span>{{ score }}</span>
      </p>
    </header>
    ...
    <div class="container">
      <div v-if="success" class="mask">
        <h1>You win!</h1>
        <button class="init-button" @click="init">Try again</button>
      </div>
...
<script>
import Background from '@/views/other/game/Background'
export default {
  name: 'Home',
  components: {
    Background
  ...
  methods: {
    ...
    generateOneNumberCell() {
      ...
      const isExist = () => this.getCellByPoint({ x: newCell.x, y: newCell.y })
      while (isExist()) {
        ...
        for (let j = 0; j < row.length; j++) {
          ...
          if (j === 0) {
            ...
            if (row[j].num === row[j - 1].num && !visited) {
              ...
              this.canMove = false
              // 获取当前数字格的DOM
              const dom1 = document.querySelector(`#c${row[j].id}`)
              ...
              dom1.addEventListener(
                ...
                () => {
                  ...
                  const dom2 = document.querySelector(`#c${row[j - 1].id}`)
        ...
        for (let j = 0; j < column.length; j++) {
          ...
          if (j === 0) {
            ...
            if (column[j].num === column[j - 1].num && !visited) {
              ...
              const dom1 = document.querySelector(`#c${column[j].id}`)
              dom1.addEventListener(
                'transitionend',
                () => {
                  ...
                  const dom2 = document.querySelector(`#c${column[j - 1].id}`)
                  this.animateMerge(dom2)
